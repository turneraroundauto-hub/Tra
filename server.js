...
// ── TEMP: CREDIT RESET ENDPOINT (remove after use) ───────────────
app.get("/admin/reset-credits", async (req, res) => {
  const email = req.query.email;
  const tier  = req.query.tier || "starter";
  if (!email) return res.status(400).json({ error: "email required" });

  const key = `sub:${email}`;
  const tierConfig = credits.TIERS[tier] || credits.TIERS.starter;

  // Force reset the credit record
  const newRecord = {
    tier,
  ...
  require('fs').writeFileSync(
    require('path').join(__dirname, 'credits.json'),
    JSON.stringify({ [key]: newRecord }, null, 2)
  );
  ...
  res.json({
    ...
    key,
    record: newRecord,
    message: `Credits reset: ${tier} tier with ${tierConfig.monthlyCredits} credits`,
  });
});
