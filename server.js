...
// ─── FETCH QUOTE via Stooq (no auth required) ────────────────────
async function fetchQuote(symbol) {
  try {
    // Stooq uses different symbol format for some tickers
    const stooqSymbol = symbol
      .replace("BTC-USD", "btc.v")
      .replace("^", "")
      .toLowerCase();
    ...
    if (!close || !open) throw new Error("Invalid data");
...
// ─── HEALTH CHECK ─────────────────────────────────────────────────
app.get("/", (req, res) => {
  res.json({ status: "ok", service: "Trade Verdict API", version: "1.3.0" });
});

// ─── MARKET DATA ENDPOINT ─────────────────────────────────────────
app.get("/market", async (req, res) => {
  try {
    const symbols = ["spy", "qqq", "btc.v", "iwm", "soxx", "xbi"];
    const labels  = ["spy", "qqq", "btc",   "iwm", "soxx", "xbi"];
    ...
    const data = {};
    results.forEach((r, i) => {
      data[labels[i]] = r.status === "fulfilled" && r.value
        ? r.value
...
app.listen(PORT, () => {
  console.log(`Trade Verdict API v1.3.0 running on port ${PORT}`);
