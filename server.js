...
// ─── FETCH QUOTE via Yahoo Finance v8 (working endpoint) ─────────
async function fetchQuote(symbol) {
  try {
    ...
    if (!price || !prev) throw new Error("No price data");
...
// ─── HEALTH CHECK ─────────────────────────────────────────────────
app.get("/", (req, res) => {
  res.json({ status: "ok", service: "Trade Verdict API", version: "1.5.0" });
});
...
app.listen(PORT, () => {
  console.log(`Trade Verdict API v1.5.0 running on port ${PORT}`);
