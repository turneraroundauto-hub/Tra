...
async function getSubscriber(email) {
  ...
  try {
    // Case-insensitive match, tolerate duplicate rows
    const { data, error } = await supabase
      .from("subscribers")
      .select("*")
      .ilike("email", email.trim());
    if (error) { console.error("Subscriber lookup error:", error.message); return null; }
    if (!data || data.length === 0) { console.log(`Subscriber not found: ${email}`); return null; }
    if (data.length > 1) console.warn(`DUPLICATE subscriber rows for ${email}: ${data.length} — using best`);
    // Prefer active paid row over free/inactive
    ...
    return data[0];
  } catch(e) { console.error("Subscriber lookup exception:", e.message); return null; }
}
  ...
  switch(event.type) {
    ...
    case "customer.subscription.updated":
      if (tierInfo && email) {
        ...
        await credits.upgradeTier(tierInfo.key + ":" + email, tierInfo.tier);
...
app.post("/stripe/credits", express.raw({ type: "application/json" }), async (req, res) => {
