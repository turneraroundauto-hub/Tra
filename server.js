...
// ─── FETCH QUOTE via Finnhub (free, no auth for quotes) ──────────
async function fetchQuote(symbol) {
  try {
    // Use Finnhub free tier - no API key needed for basic quotes
    ...
    // Finnhub response: c=current, pc=previous close, d=change, dp=change%
    ...
    const pct  = changePct || ((current - prevClose) / prevClose) * 100;
    const sign = pct >= 0 ? "+" : "";
...
// ─── HEALTH CHECK ─────────────────────────────────────────────────
app.get("/", (req, res) => {
  res.json({ status: "ok", service: "Trade Verdict API", version: "1.4.0" });
});
...
Live market data:
- SPY: ${sectorContext?.spyChange || "?"}
- QQQ: ${sectorContext?.qqqChange || "?"}
- BTC: ${sectorContext?.btcChange || "?"}
...
app.listen(PORT, () => {
  console.log(`Trade Verdict API v1.4.0 running on port ${PORT}`);
  console.log(`API key configured: ${!!process.env.ANTHROPIC_API_KEY}`);
  // Log first 8 chars of key to verify it loaded (safe to log)
